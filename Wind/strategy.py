...
class ModularIntradayStrategy:
    ...
    def can_reenter(self, current_price, timestamp, current_bar_data):
        """
        Simplified exit-reason specific re-entry logic:
        - After stop-loss: Wait for fresh signal (ignore previous conditions)
        - After time exit: No re-entry same day
        - After profit-taking: Immediate re-entry allowed
        """
        # If no previous exit, allow entry
        ...
        if self.last_time_exit_date is not None and timestamp.date() > self.last_time_exit_date:
            self.last_exit_reason = ""  # Reset for new trading day
            self.last_time_exit_date = None
            return True
        
        # Exit-reason specific logic
        if self.last_exit_reason == "time":
            # After time exit: No re-entry same day
            if self.last_time_exit_date == timestamp.date():
                return False
            else:
                return True
        
        elif self.last_exit_reason in ["Base Stop Loss", "Trail Stop Loss"]:
            # After stop-loss: Wait for fresh signal - just need basic bullish conditions
            # No complex momentum checks or price buffers
            
            # Check if we have fresh bullish alignment
            fresh_signal = True
            
            # Check EMA crossover if enabled
            if self.use_ema_crossover:
                if not current_bar_data.get('ema_bull', False):
                    fresh_signal = False
            
            # Check VWAP if enabled
            if self.use_vwap:
                if not current_bar_data.get('vwap_bull', False):
                    fresh_signal = False
            
            # Check MACD if enabled
            if self.use_macd:
                macd_histogram = current_bar_data.get('macd_histogram', 0)
                if not (macd_histogram > 0):
                    fresh_signal = False
            
            return fresh_signal
        
        elif self.last_exit_reason == "profit":
            # After profit-taking: Immediate re-entry allowed
            return True
        
        else:
            # For any other exit reason, allow re-entry
            return True
    ...
    def on_tick(self, tick_timestamp, tick_price, tick_volume):
        ...
        if self.indicator_manager.is_ema_positive_signal(self.ema_points_threshold):
            # Generate buy signal or take action
            pass
