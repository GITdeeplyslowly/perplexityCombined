...
class Results:
    ...
    def export_to_excel(self, output_dir: str = "results") -> str:
        """Export to Excel with config as a table, trade table split for visibility, and font size 16+."""
        os.makedirs(output_dir, exist_ok=True)
        trades_df = self.get_trade_summary()
        additional_info_df = self._create_additional_info_table()

        # Transform config to a wide table: keys as columns, one row
        ...
        # Break the trades table into two: first 7 cols, remaining cols
        trade_cols = trades_df.columns.tolist()
        break_idx = 7  # show first 7 columns then rest
        ...
        with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
            config_wide_df.to_excel(writer, sheet_name="Sheet1", index=False, startrow=0)
            trades_df_1.to_excel(writer, sheet_name="Sheet1", index=False, startrow=3)  # config table + 2 row gap
            trades_df_2.to_excel(writer, sheet_name="Sheet1", index=False, startrow=3+len(trades_df_1)+2)  # Under first table, gap
        ...
        # Style all headers: bold, center
        header_rows = [4, 4 + len(trades_df_1) + 2]
        ...
        # Number formatting for numeric columns (both tables)
        ...
        # Find data row bounds
        table1_start = 5
        table1_end = table1_start + len(trades_df_1) - 1
        table2_start = 5 + len(trades_df_1) + 2
        table2_end = table2_start + len(trades_df_2) - 1

        # Format both tables
        for header_row, data_start, data_end in [
            (4, table1_start, table1_end),
            (4 + len(trades_df_1) + 2, table2_start, table2_end)]:
        ...
        # (Optional) add color coding for Gross P&L if present in first table
        gross_pnl_cols = []
        headers_1 = [ws.cell(row=4, column=col).value for col in range(1, ws.max_column + 1)]
