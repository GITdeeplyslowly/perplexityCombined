...
class ForwardTestResults:
    ...
    def export_to_excel(self, test_type: str = "live") -> str:
        ...
        # --- HYBRID CONFIG SHEET EXPORT (fail-safe, raw text) ---

        # Split dialog text into lines and create a simple DataFrame

        if self.dialog_text:

            lines = self.dialog_text.splitlines()

            config_df = pd.DataFrame({"Configuration": lines})

            

            # Append as a new sheet named "Config"
            ...
            logger.info(f"✅ Config sheet added with {len(lines)} lines of dialog text")

        else:

            logger.warning("⚠️ No dialog_text available - Config sheet not created")
        ...
        logger.info("File contains: Enhanced Excel report with all consolidated information")
    ...
    def get_summary_metrics(self) -> List[Tuple[str, str]]:
        ...
        metrics = [
            ...
            ("Winning Trades", str(perf.get('winning_trades', 0))),

            ("Losing Trades", str(perf.get('losing_trades', 0))),

            ("Win Rate", f"{perf.get('win_rate', 0):.2f}%"),

            ("Gross P&L", f"{perf.get('total_pnl', 0):,.2f}"),

            ("Net P&L", f"{calculated_net_pnl:,.2f}"),

            ("Profit Factor", f"{perf.get('profit_factor', 0):.2f}"),

            ("Best Trade", f"{perf.get('max_win', 0):,.2f}"),

            ("Worst Trade", f"{perf.get('max_loss', 0):,.2f}"),

            ("Initial Capital", f"{initial_capital:,.2f}"),

            ("Final Capital", f"{initial_capital + calculated_net_pnl:,.2f}"),

            ("Return %", f"{(calculated_net_pnl/initial_capital*100):,.2f}%" if initial_capital > 0 else "0.00%")
    ...
    def _get_dialog_box_text(self) -> str:
        ...
        Just paste what the user saw.

        """

        logger.info(f"🔍 _get_dialog_box_text called - dialog_text type: {type(self.dialog_text)}, length: {len(self.dialog_text) if self.dialog_text else 0}")

        

        if self.dialog_text:

            logger.info(f"✅ dialog_text exists - first 100 chars: {self.dialog_text[:100]}")

        else:

            logger.error(f"❌ dialog_text is empty or None: {repr(self.dialog_text)}")
    ...
    def _create_dashboard_sections(
        ...
        # Create section header using layout helper method

        cell_ref = layout_manager.merge_cells_range(start_col, end_col)

        header_cell = ws[cell_ref]

        header_cell.value = "STRATEGY CONFIGURATION"

        header_cell.font = style_manager.fonts['header']

        header_cell.fill = style_manager.fills['header']

        header_cell.alignment = style_manager.center_align

        ws.row_dimensions[layout_manager.current_row].height = 25

        layout_manager.advance_row(1)
        ...
        # DEBUG: Log the actual dialog text being written

        logger.info("=" * 80)

        logger.info("DIALOG TEXT TO BE WRITTEN TO EXCEL:")

        logger.info("=" * 80)

        logger.info(f"Type: {type(dialog_text)}, Length: {len(dialog_text)}")

        logger.info(f"First 200 chars: {dialog_text[:200]}")

        logger.info("=" * 80)

        

        # DYNAMIC SIZE CALCULATION: Calculate rows needed based on actual line count

        lines_in_text = dialog_text.count('\n') + 1

        

        # Each Excel row can comfortably fit ~2 lines of text (with wrapping at 10pt font)

        # Add 20% buffer to ensure no text is cut off

        estimated_rows_needed = max(20, int(lines_in_text * 0.6))  # 0.6 = ~1.67 lines per Excel row with buffer

        

        logger.info(f"📏 Dynamic sizing: {lines_in_text} text lines → {estimated_rows_needed} Excel rows")

        

        # Create merged cell with dialog content

        content_start_row = layout_manager.current_row

        content_end_row = content_start_row + estimated_rows_needed

        

        # CRITICAL: Set formatting BEFORE merging for proper Excel rendering

        config_cell = ws.cell(row=content_start_row, column=start_col)

        config_cell.value = dialog_text

        config_cell.alignment = Alignment(wrap_text=True, vertical="top", horizontal="left")

        config_cell.font = Font(name='Courier New', size=10, bold=True, color='000000')

        config_cell.fill = PatternFill(start_color='FFFFFF', end_color='FFFFFF', fill_type='solid')

        config_cell.border = Border(
        ...
        # DEBUG: Verify what was written before merging

        logger.info(f"📝 Cell value BEFORE merge - Type: {type(config_cell.value)}, Length: {len(str(config_cell.value))}")

        logger.info(f"📍 Cell location: {config_cell.coordinate}")

        

        # Now merge cells AFTER formatting and writing

        content_merge_range = f"{ws.cell(row=content_start_row, column=start_col).coordinate}:{ws.cell(row=content_end_row, column=end_col).coordinate}"

        ws.merge_cells(content_merge_range)

        logger.info(f"✅ Merged range: {content_merge_range}")

        

        # Set row heights dynamically based on content

        # Formula: Distribute total text height across allocated rows

        # At 10pt font, each line needs ~15 pixels; add padding

        pixels_per_text_line = 18  # 10pt font + line spacing

        total_height_needed = lines_in_text * pixels_per_text_line

        height_per_excel_row = max(25, total_height_needed / estimated_rows_needed)

        

        logger.info(f"📏 Row height calculation: {total_height_needed}px total / {estimated_rows_needed} rows = {height_per_excel_row:.1f}px per row")

        

        for r in range(content_start_row, content_end_row + 1):

            ws.row_dimensions[r].height = height_per_excel_row

        

        # VERIFY: Re-read the cell value to confirm it was written

        verify_cell = ws.cell(row=content_start_row, column=start_col)

        logger.info(f"✅ VERIFICATION - Cell {verify_cell.coordinate} value length: {len(str(verify_cell.value)) if verify_cell.value else 0}")

        logger.info(f"✅ VERIFICATION - First 100 chars: {str(verify_cell.value)[:100] if verify_cell.value else 'EMPTY'}")

        

        # Update layout position

        layout_manager.current_row = content_end_row + 2
