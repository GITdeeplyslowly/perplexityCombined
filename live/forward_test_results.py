...
class ForwardTestResults:
    ...
    def get_summary_metrics(self) -> List[Tuple[str, str]]:
        ...
        # Metrics arranged in 2-column layout (left-right pairs per row)
        # User requested: Total Trades, Losing Trades, Gross P&L, Profit Factor, Worst Trade, Final Capital → Column C (right side)
        metrics = [

            ("Winning Trades", str(perf.get('winning_trades', 0))),  # Left
            ("Total Trades", str(perf.get('total_trades', 0))),      # Right (moved)

            ("Win Rate", f"{perf.get('win_rate', 0):.2f}%"),         # Left
            ("Losing Trades", str(perf.get('losing_trades', 0))),    # Right (moved)

            ("Net P&L", f"{calculated_net_pnl:,.2f}"),               # Left
            ("Gross P&L", f"{perf.get('total_pnl', 0):,.2f}"),       # Right (moved)

            ("Best Trade", f"{perf.get('max_win', 0):,.2f}"),        # Left
            ("Profit Factor", f"{perf.get('profit_factor', 0):.2f}"), # Right (moved)

            ("Initial Capital", f"{initial_capital:,.2f}"),          # Left
            ("Worst Trade", f"{perf.get('max_loss', 0):,.2f}"),      # Right (moved)

            ("Return %", f"{(calculated_net_pnl/initial_capital*100):,.2f}%" if initial_capital > 0 else "0.00%"),  # Left
            ("Final Capital", f"{initial_capital + calculated_net_pnl:,.2f}"),  # Right (moved)
